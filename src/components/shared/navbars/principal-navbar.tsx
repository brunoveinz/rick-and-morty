...
export default function PrincipalHeader() {
  ...
  return (
    <header className="bg-mortyOrange shadow-sm">
      <nav className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8" aria-label="Top">
        <div className="flex h-16 items-center justify-between">
          <div className="flex items-center">
            <Link href="/" className="flex items-center">
              <span className="sr-only">Tu Empresa</span>
              <span className="ml-2 text-3xl font-rick text-spaceGray">Rick and Morty</span>
            </Link>
          </div>
          <div className="hidden space-x-8 sm:flex">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-spaceGray font-rick text-gray-500 hover:text-gray-900"
              >
                {item.name}
              </Link>
            ))}
